...
pub fn build_config(
    ...
    cfg.sidebar_icons = vec![
        ...
        SidebarIcon { key: "sidebar_icon_search",     glyph: PH_SEARCH,     sidebar_id: ICON_ID_SEARCH,     opens_panel: false },
        SidebarIcon { key: "sidebar_icon_users",      glyph: PH_USERS,      sidebar_id: ICON_ID_USERS,      opens_panel: false },
        SidebarIcon { key: "sidebar_icon_terminal",   glyph: PH_TERMINAL,   sidebar_id: ICON_ID_TERMINAL,   opens_panel: false },
        SidebarIcon { key: "sidebar_icon_git",        glyph: PH_GIT,        sidebar_id: ICON_ID_GIT,        opens_panel: false },
        SidebarIcon { key: "sidebar_icon_extensions", glyph: PH_EXTENSIONS, sidebar_id: ICON_ID_EXTENSIONS, opens_panel: false },
    ];
...
pub fn setup(cv: &mut Canvas, cw: f32, ch: f32, cfg: &SetupConfig) {
    mount(cv, cw, ch, cfg);
}

pub fn handle_press(cv: &mut Canvas, mx: f32, my: f32, cfg: &SetupConfig) -> bool {
    on_press(cv, mx, my, cfg)
}

pub fn handle_release(cv: &mut Canvas, cfg: &SetupConfig) {
    on_release(cv, cfg);
}

pub fn handle_move(cv: &mut Canvas, mx: f32, my: f32) {
    on_move(cv, mx, my);
}

pub fn tick(cv: &mut Canvas, cfg: &SetupConfig) -> Panels {
    update(cv, cfg)
}