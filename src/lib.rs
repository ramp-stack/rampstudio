...
impl App {
    pub fn new(context: &mut Context, assets: Assets) -> Scene {
        ...
        let font_bold_b = assets.get_font("JetBrainsMono-Bold.ttf").expect("bold font");
        let font_reg_b  = assets.get_font("JetBrainsMono-Regular.ttf").expect("regular font");
        ...
        logic::settings::load(&mut editor_settings, &mut ex_settings_init, &mut term_settings_init);
        ...
        let ed = Arc::new(Editor::new(
            ...
            init_split_b - init_split_a - DIV_W, INIT_CH - panel_top,
            code_font.clone(), gutter_font.clone(),
            &initial_file, &theme_bytes,
            editor_settings,
        ));
        ...
        let ex_component = explorer_mount(
            ...
            font_bold_b.clone(), font_reg_b.clone(), font_fa_b,
        ...
        let _term = terminal_mount(
            ...
            move |raw_cmd, t| { run_command(raw_cmd, t, &cwd_cmd); },
        );
        ...
        scene.get_layer_mut(layer_id).unwrap().canvas_mut()
            .on_update(move |cv| {
                ...
                { let mut es = ex_settings_upd.get_mut(); es.x = p.explorer.0; es.y = p.explorer.1; es.w = p.explorer.2; }
